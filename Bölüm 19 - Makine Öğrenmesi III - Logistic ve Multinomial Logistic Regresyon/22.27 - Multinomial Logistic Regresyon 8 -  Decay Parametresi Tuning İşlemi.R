...
modelTuning <- train( cp ~ thalach + exang + oldpeak + slope + ca + thal  , 
                      data = train,
                      method = "multinom",
...
modelTuned <- multinom(cp ~ thalach + exang + oldpeak + slope + ca + thal , 
                       data  = train ,
                       decay = 0.1
                      )
...
caret::confusionMatrix(predModelTuned , testOranlarEsit$cp , mode = "prec_recall")


# Tüm değişknler üzerinden 

modelTuningAllVar <- train( cp ~ . , 
                      data = train,
                      method = "multinom",
                      trControl = trainControl(method = "cv" , number = 5)
)
...
plot(modelTuningAllVar)

modelTuningAllVar$bestTune
